...
//user - create new publish
exports.user_registration_publisher = async (req, res) => {
  try {
    const user = new Publisher(req.body);
    const email = await Publisher.findOne({ email: req.body.email });
    if (email) {
      return res.status(400).send({
        type: "error",
        message:
    ...
    user.masterPublisher = req.user._id;
    user.userId = idGenerator(20);
    randomNumber = Math.floor(Math.random() * (20000 * 30000));
    user.userName =
      req.body.name.toLowerCase().trim().replace(/\s/g, "") + randomNumber;
...
exports.user_masterpublisher_publisher = async () => {
  try {
    const users = await Publisher.find({ masterPublisher: req.user._id });
    //limit
    res.send({ users });
  } catch (error) {
    res.status(500).send(error);
...
//delete publisher
exports.user_masterpublisher_publisher_block = async () => {
  try {
    const user = await Publisher.findOne({ _id: req.params.id });
    user.status = "BLOCKED";
    await user.save();
    res.send(user);
  } catch (error) {
